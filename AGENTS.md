...
> # 🔴 READ THESE THREE FIRST — EVERY AGENT, EVERY SESSION, BEFORE ANY WORK
> 1. **The Koydo Charter** — `~/koydo-design/AGENT_CHARTER.md`
> 2. **Autonomous execution protocol** — `~/koydo-design/principles/autonomous-agent-execution.md`
> 3. **Koydo Brain** — `~/koydo-brain/` (start at `README.md` + `INDEX.md`)
>
> They are the source of truth. Everything below, and every other file in this repo, defers to them.
> Do not guess or stop at a search hit. Resolve uncertainty through canon, code, consumers, Git, runtime,
> and provider state; proceed on reversible in-scope work and ask only for a genuine owner-only fork.

> Provider-neutral cross-agent design + governance contract. The Charter and autonomous-execution principle are canonical; provider-specific files are adapters only. Keep this file in sync.
...
1. `~/koydo-design/AGENT_CHARTER.md` — provider-neutral highest Koydo authority below live owner directives; never hardcode a version here.
2. `~/koydo-design/principles/autonomous-agent-execution.md` — standing permissions, evidence ladder, live TASKS leases, Git/cloud discipline, and communication.
3. **This file (`AGENTS.md`)** — repo-local rules and canon pin.
...
If a component is absent, prove the gap by reading canon and consumers, then extend canon or file the scoped task/issue. Do not invent a local fork and do not bounce a reversible canon fix back to the owner.
...
Per the Charter and `principles/autonomous-agent-execution.md`:

- ✅ **Commit + push every coherent, locally verified task slice** to `origin/master` (Koydo/ org only), staging only lane-owned paths. Verify upstream parity after the push.
- ❌ **No cloud builds.** No GitHub Actions build/test/deploy, remote codegen, or source upload that starts a build. All builds, tests, lints, type checks, codegen, and artifact verification run locally.
- ✅ **Cloud deployments are allowed after the exact local build and required local gates succeed.** Vercel receives `vercel deploy --prebuilt --archive=tgz`; the app's launch rule still controls production alias/traffic changes. Workers are bundled and deployed with Wrangler after local gates.
- ✅ **Deep commit messages** identify the task/lane, why, behavior/config changed, verification, risk/follow-up, and required attribution for the actual contributing agent/model.
- ✅ **Cloud placement:** files/media/blobs → R2; relational/content rows → Supabase; new actions/webhooks/queues/cron → Cloudflare Workers/Wrangler; GitHub → source control only.
...
actions, and `TASKS/3-BLOCKED/` for blocked work. Before any write, create or
renew a path-bounded `TASKS/CLAIMS.md` live lease with RFC 3339 local timestamps,
machine/session, task ID, repo/branch/base SHA, exact paths/resources, exclusions,
heartbeat, and expiry. An expired lease requires a stale-work completion audit;
it never authorizes overwriting or deleting another lane's work.
